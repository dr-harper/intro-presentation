...
knitr::include_graphics("https://github.com/dr-harper/intro-presentation/blob/master/images/phdWordcloud.png?raw=true")
```
...
$$\text{find } n\ \text{where}\ \sum_{j=1}^{n} E_{Demand} - E_{Supply} = 0$$
...
p <-
  ...
  ggrepel::geom_label_repel(data = interests, aes(Skill, Enjoyment, label = Item), nudge_x = 0.4, alpha = 0.7) +
...
This presentation was brought to you by R Markdown
